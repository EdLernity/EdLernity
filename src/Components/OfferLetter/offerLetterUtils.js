...
function getTemplateType(pdfUrl) {
  const match = OFFER_LETTER_TEMPLATES.find((item) => item.url === pdfUrl);
  return match?.type || 2;
}

async function stampOfferLetterPdf(pdfUrl, name) {
...
export async function sendOfferLetterEmail({ pdfUrl, email, name, mailBody, subject }) {
  ...
  const { data } = await apiInstancePrivate.post("/api/v1/course/offer-letter", formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
  return data;
}

export async function downloadOfferLetterPdf({ pdfUrl, name, fileName }) {
  const modifiedPdfBytes = await stampOfferLetterPdf(pdfUrl, name);
