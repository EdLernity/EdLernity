          ...
          {certificates.map((row) => {
            ...
            return (
              ...
              >
                ...
                {active && row.uuid ? (
                  <div className="flex flex-wrap gap-2 border-t border-brand-100 px-4 py-2 dark:border-brand-500/20">
                    <a
                      href={`https://edlernity.com/credential/${encodeURIComponent(row.uuid)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="rounded-lg bg-brand-50 px-2.5 py-1 text-[11px] font-semibold text-brand-700 hover:bg-brand-100 dark:bg-brand-500/10 dark:text-brand-300"
                      onClick={(e) => e.stopPropagation()}
                    >
                      Share credential page
                    </a>
                    <a
                      href={`https://edlernity.com/verify-certificate?id=${encodeURIComponent(row.uuid)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="rounded-lg bg-gray-50 px-2.5 py-1 text-[11px] font-semibold text-gray-600 hover:bg-gray-100 dark:bg-white/5 dark:text-gray-300"
                      onClick={(e) => e.stopPropagation()}
                    >
                      Verify link
                    </a>
